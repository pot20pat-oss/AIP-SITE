...
async function handleBot(request, env, ctx) {
  ...
  const INJECT = /(?:ignore[rs]?|oubli[es]?|oublie|bypass|contourne?|depasse?|réécri[s]?|reecri[s]?|supplan[te]?|forget)(?:.{0,40}?(?:tes|all|previous|ton|mes|les|toutes)?\s*?(?:instructions?|ordres?|règles?|regles|prompt|system|systeme))|(?:mode\s+(?:dev|developpeur|développeur|admin|debug|administrateur))|(?:répète|repete|repeat|répéter|repeats?|reproduis|recite|récris?|recopie|copie|dis[- ]?moi|montre[- ]?moi|cite?|print|dump|reveal|révéle?|show me|donne[- ]?moi|ecris|écris)(?:.{0,80}?(?:prompt|system|configuration|cachée?|caché|secret|instruction|systeme|instructions?|complet|intégral|intégralement|mot pour mot|textuellement))|(?:ton|tes|the|your|mon|ma)(?:.{0,40}?(?:prompt|system|instructions?|config|configuration))/i;
  if (INJECT.test(msg)) {
